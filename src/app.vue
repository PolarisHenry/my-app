<template>
  <div class="Wokoo" v-if="show">
    <header class="Wokoo-header">
      ...
      <span class="Wokoo-close-icon" @click="handleClose"> × </span>
      <p>Edit <code>App.vue</code> and save to reload.</p>
      <a
        class="Wokoo-link"
  ...
  <div class="Wokoo-hide" @click="handleClose" v-else>
    ...
    open
  <span v-for="item in messages" :key="item.message">{{ item.message}}</span>
  </div>
</template>
<script>
import "./app.less";
